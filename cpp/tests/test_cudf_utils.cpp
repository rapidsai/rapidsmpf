...
using ColumnTypes = ::testing::
    Types<std::int8_t, std::int16_t, std::int32_t, std::int64_t, float, double, bool>;
...
TYPED_TEST(EstimatedMemoryUsageTest, FixedWidthColumnMemoryUsage) {
    ...
    for (auto size : test_sizes) {
        ...
        std::size_t est = rapidsmpf::estimated_memory_usage(column->view(), this->stream);
...
TEST_F(BaseEstimatedMemoryUsageTest, StringType) {
    ...
    for (const auto& data : test_cases) {
        ...
        std::size_t exp = column->alloc_size();
        std::size_t est = rapidsmpf::estimated_memory_usage(column->view(), stream);
...
TEST_F(BaseEstimatedMemoryUsageTest, StructType) {
    ...
    for (const auto& data : test_cases) {
        ...
        cudf::test::fixed_width_column_wrapper<std::int32_t> int_wrapper(
