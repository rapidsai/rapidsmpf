 ...
 *
 * @note The return values of `coro::when_all` and `coro::when_any` must always
 * be retrieved by calling `.return_value()` (either directly or via this helper).
 * Failing to do so leaves exceptions unobserved, which can cause the streaming
 * pipeline to deadlock or hang indefinitely while waiting for error propagation.
 */
