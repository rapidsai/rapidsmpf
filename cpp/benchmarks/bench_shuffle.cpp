...
[[nodiscard]] PackedData make_chunk(
    ...
    auto reservation = [&]() -> MemoryReservation {
        if (args.input_memory_type == MemoryType::DEVICE) {
            auto [device_reservation, _] =
                br.reserve(MemoryType::DEVICE, size, AllowOverbooking::YES);
