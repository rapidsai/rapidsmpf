...
streaming::Node consumer(
    streaming::Context& ctx, streaming::SharedChannel<streaming::TableChunk> ch_in
) {
...
int main(int argc, char** argv) {
    ...
    auto br = std::make_shared<BufferResource>(mr, std::move(memory_available));
    ...
    unsigned int coro_thread_count = 4;
    streaming::Context ctx(options, comm, stream, br, coro_thread_count, stats);
