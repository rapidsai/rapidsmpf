/*
 * SPDX-FileCopyrightText: Copyright (c) 2025-2026, NVIDIA CORPORATION. All rights
...
streaming::Node build_bloom_filter(
    ...
    if (ctx->comm()->nranks() > 1) {
        ...
        auto const net_memory_delta = static_cast<std::int64_t>(filter->size());
        auto buf = ctx->br()->allocate(
            stream,
            co_await ctx->memory(MemoryType::DEVICE)
                ->reserve_or_wait_or_fail(filter->size(), net_memory_delta)
        );
