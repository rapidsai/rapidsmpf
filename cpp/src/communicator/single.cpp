...
std::pair<std::unique_ptr<std::vector<std::uint8_t>>, Rank> Single::recv_any(Tag) {
    return {nullptr, 0};
}

std::unique_ptr<std::vector<std::uint8_t>> Single::recv_from(Rank, Tag) {
    return nullptr;
}
...
std::unique_ptr<std::vector<std::uint8_t>> Single::release_sync_host_data(
