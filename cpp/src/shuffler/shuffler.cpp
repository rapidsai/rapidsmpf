...
bool Shuffler::progress() {
    ...
    for (auto&& chunk : inbox_.extract_all()) {
        ...
        fire_and_forget_.push_back(
            comm_->send(chunk.to_metadata_message(), dst, metadata_tag, stream_, br_)
        );
         ...
         first_chunk != incoming_chunks_.end();)
    {
        ...
        if (chunk.gpu_data_size > 0) {
            ...
            auto future = comm_->recv(src, gpu_data_tag, std::move(recv_buffer), stream_);
            RAPIDSMP_EXPECTS(
    ...
    while (true) {
        ...
        if (msg) {
            ...
            statistics_->add_bytes_stat("shuffle-payload-send", chunk.gpu_data->size);
            fire_and_forget_.push_back(
                comm_->send(std::move(chunk.gpu_data), src, gpu_data_tag, stream_)
            );
        } else {
            break;
        }
    ...
    if (!fire_and_forget_.empty()) {
        ...
        for (auto i : finished) {
            fire_and_forget_.erase(fire_and_forget_.begin() + i);
        }
    }
    stats.add_duration_stat(
