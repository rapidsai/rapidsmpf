...
def main() -> None:
    ...
    if not CUPTI_AVAILABLE:
        print("CUPTI support is not available. Please ensure CUPTI support is compiled in.")
        return
    ...
    try:
        ...
        for i in range(num_allocations):
            print(f"Allocating {allocation_size // (1024 * 1024)} MB on GPU using rmm.DeviceBuffer...")
            try:
        ...
        if samples:
            ...
            initial_utilization = (initial_sample.used_memory / initial_sample.total_memory) * 100.0
            final_utilization = (final_sample.used_memory / final_sample.total_memory) * 100.0

            print(f"Initial memory usage: {initial_sample.used_memory / (1024.0 * 1024.0):.2f} MB "
                  f"({initial_utilization:.1f}%)")
            print(f"Final memory usage: {final_sample.used_memory / (1024.0 * 1024.0):.2f} MB "
            ...
            for sample in samples:
                if sample.used_memory > peak_used:
                    ...
                    peak_utilization = (sample.used_memory / sample.total_memory) * 100.0

            print(f"Peak memory usage: {peak_used / (1024.0 * 1024.0):.2f} MB "
