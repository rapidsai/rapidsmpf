...
def parse_disableable_option(value, *, default_value):
    """
    Parses a string into a typed value, with support for disabling.

    Designed for use as a `factory` argument to `Options.get()`, this function
    interprets a string input as either:

    - A valid value of the same type as `default_value`, or
    - A special "disable" keyword, which causes the option to resolve to `None`.

    If the input string is empty, `default_value` is returned. If the input is
    a recognized disable keyword, `None` is returned instead.

    Recognized disable keywords (case-insensitive):
        - "false"
        - "no"
        - "off"
        - "disable"
    ...
    Otherwise, the input string is cast to the type of `default_value`.

    Parameters
    ----------
    value
        The string representation of the config option.
    default_value
        The default value to use if `value` is empty. Also used to determine the
        target type when casting non-empty values.

    Returns
    -------
    Returns `None` if `value` is a disable keyword. Otherwise, returns the
    value cast to the type of `default_value`.

    Raises
    ------
    ValueError
        If the input value is not a recognized disable keyword and cannot be
        converted to the type of `default_value`.

    Examples
    --------
    Used in conjunction with `Options.get()`:

    >>> Options().get(
    ...     "dask_periodic_spill_check",
    ...     return_type=object,
    ...     factory=partial(parse_disableable_number, default_value=1e-3),
    ... )
    0.001  # if unset

    >>> parse_disableable_number("0.1", default_value=1e-3)
    0.1

    >>> parse_disableable_number("off", default_value=1e-3) is None
    True

    >>> parse_disableable_number("", default_value=1e-3)
    0.001

    Notes
    -----
    The input string must be convertible using ``type(default_value)(value)``.
    If not, a ``ValueError`` or ``TypeError`` may be raised at runtime.

    The option is parsed as a Python object and isn't accessible from C++.
    """
    if not value:
        return default_value

    if value.strip().lower() in {"false", "no", "off", "disable", "disabled"}:
        return None
