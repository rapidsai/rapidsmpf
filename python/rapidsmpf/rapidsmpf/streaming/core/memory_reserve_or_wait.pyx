...
cdef extern from * nogil:
    ...
    void cpp_mrow_reserve_or_wait_or_overbook(
        std::shared_ptr<rapidsmpf::streaming::MemoryReserveOrWait> mrow,
        std::size_t size,
        std::size_t net_memory_delta,
        std::pair<std::unique_ptr<rapidsmpf::MemoryReservation>, std::size_t> &output,
...
cdef class MemoryReserveOrWait:
    ...
    reservation request. Instead, it bounds how int64_t the system may go without
    ...
    def __init__(
        ...
        self._br = ctx.br()
        with nogil:
            self._handle = cpp_mrow_make_shared(
                options._handle,
                mem_type,
                deref(ctx._handle),
            )
    ...
    async def reserve_or_wait_or_fail(self, size_t size, *, int64_t net_memory_delta):
        ...
        Py_INCREF(ret)
        with nogil:
            cpp_mrow_reserve_or_wait_or_fail(
                self._handle,
                size,
                net_memory_delta,
                c_ret,
