...
int main(int argc, char** argv) {
    // In this example we use the MPI backend. For convenience, rapidsmpf provides an
    // optional MPI-init function that initializes MPI with thread support.
    ...
    // maps partitions to their destination ranks. All ranks must use the same owner
    ...
    // Again, this is non-blocking and should be done as soon as we know that we don't
