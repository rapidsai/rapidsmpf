...
Buffer
  A raw memory allocation that can reside in device (GPU), pinned host, or regular host memory. Buffers have an attached CUDA stream maintained for the lifetime of the buffer. They are created through a {term}`BufferResource`.
...
Channel
  An asynchronous messaging queue used for communicating {term}`Message`s between {term}`Node`s in a streaming network. Channels provide backpressure to prevent excessive memory consumption by suspending senders when full. They do not copy, serialize, or spill data - they simply pass references between nodes.

Collective Operation
  A communication pattern that involves coordination across multiple {term}`Rank`s. Examples include {term}`Shuffler` (redistributing data by partition), {term}`AllGather` (gathering data from all ranks), and {term}`Fanout` (broadcasting to multiple channels).
