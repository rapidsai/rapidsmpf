...
| `reserve-{memtype}-waiting-requests` | Requests waiting concurrently, reported as the peak and the mean rather than a total. |
| `reserve-{memtype}-request-bytes` | Bytes requested from `reserve_or_wait()`. |
| `reserve-{memtype}-overbook-bytes` | Bytes by which `reserve_or_wait_or_overbook()` exceeded the memory limit after the timeout. |
