...
NVIDIA RapidsMPF, including its nascent Streaming Engine and Out-of-Core
(OOC) shuffle, provides a unified framework for asynchronous, multi-GPU
pipelines using simple streaming primitives, {term}`Channel`s,
{term}`Actor`s, and {term}`Message`s, built using NVIDIA CUDA-X components:
{doc}`RMM <rmm:index>` and {doc}`UCXX <ucxx:index>`.
